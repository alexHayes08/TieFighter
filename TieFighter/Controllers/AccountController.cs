...
namespace TieFighter.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpGet]
        public async Task<ActionResult> YourStats()
        {
            var user = await _userManager.GetUserAsync(User);
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            ViewBag.Error = "Something went wrong in the login process, please " +
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, CreatedOn = DateTime.Now, MostRecentActivity = DateTime.Now };
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            else
            {
                ...
                var thumbnail = info.Principal.FindFirst("picture")?.Value;
                var role = info.Principal.FindFirst(ClaimTypes.Role);
                //return View("ExternalLoginConfirmation", new ExternalLoginConfirmationViewModel { Email = email, Thumbnail = thumbnail });

                var model = new ExternalLoginConfirmationViewModel { Email = email, Thumbnail = thumbnail };
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, Thumbnail = model.Thumbnail };
                var createUserResult = await _userManager.CreateAsync(user);
                if (createUserResult.Succeeded)
                {
                    var roleName = Enum.GetName(typeof(UserRoles), UserRoles.Registered);
                    ...
                    if (createUserResult.Succeeded)
                    {
                        ...
                        var b = _signInManager.IsSignedIn(info.Principal);
                        var c = await _signInManager.CanSignInAsync(user);
